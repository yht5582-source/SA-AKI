...
Final-review history/provenance fixes: Acute CRRT is now reconciled across immutable observations for each diagnosis point. An omitted later field cannot stop an established acute episode; an explicit stop ends current-treatment status but retains stage-3 episode interpretation; conflicting/orphaned event chronology is visible provenance uncertainty with provisional/low-confidence conservative staging, never a silent downgrade. Repeated identical event timestamps are corroboration rather than separate episodes. The HA wizard now uses the shared provenance-aware `strictlyEarlierSnapshot()` selector, so a duplicate latest prior instant leaves HA incomplete and disables review completion; the rendered unique-prior path remains enabled. RED→GREEN and rationale: `history-provenance-fixes-report.md`.

Independent-review correction to the history fix: current KDIGO staging is now separated from the acute episode's historical maximum. A provenance-valid linked CRRT stop ends automatic current-RRT stage 3; the historical stage-3 maximum stays visible in evidence without claiming recovery, maintenance dialysis, or a new KRT indication. Start/stop event times must not be later than their documenting snapshot; time passing alone cannot execute a previously planned event absent later explicit reconfirmation. Same-snapshot start/stop pairs retain linkage; unlinked records and conflicting stops for one start remain visible manual-review uncertainty even after a later start, and no independently current criterion yields provisional/insufficient current staging rather than a permanent critical stage 3. Direct RED→GREEN diagnosis coverage added; details appended to `history-provenance-fixes-report.md`.

Independent-review boundary correction: confirmed CRRT pair intervals must have `stop > start` and be strictly separated (`next start > previous stop`). Overlap, an equal boundary, zero duration, and an unlinked start within or at/before a completed episode's stop now fail closed with visible manual-review missing data; none can silently establish recovery or active current RRT. Strictly sequential paired episodes remain accepted. RED→GREEN evidence: 4 new boundary failures, diagnosis 87/87 green; fresh both typechecks, lint, 26 files / 940 Vitest tests, and production build passed. Details appended to `history-provenance-fixes-report.md`.

Whole-branch review current-selection correction: deterministic diagnosis history ordering no longer overrides an explicit same-time dashboard selection. The selected snapshot ID now identifies diagnosis/sepsis current observation, while the ordered series remains for historical reconciliation; duplicate-time provenance warnings and the strictly-earlier comparator safeguard remain intact. Direct clinical, dashboard-adapter, and rendered switch regressions show `a-severe` produces stage-3/SOFA-12 cards and `z-normal` produces its own normal/SOFA-2 cards. RED→GREEN 118 focused tests; fresh both typechecks, lint, 26 files / 948 Vitest tests, and production build passed. Details appended to `history-provenance-fixes-report.md`.
